...
public class PingCMD extends WCCCmd{
    ...
    public void run(WCCUser user, String label, String[] args) {
        if (args.length == 1 && user.hasPermission("wcc.admin")) {
            Player target = plugin.getServer().getPlayer(args[0]);
            if (target == null) {
                user.sendMessage("&cEl usuario está desconectado");
                return;
            }
            user.sendMessagePrefix("Ping de &2" + target.getDisplayName() + "&r: " + format(WCCServer.getUser(target).getPing()));
            return;
        }
        if (user.getPing() < 0) {
